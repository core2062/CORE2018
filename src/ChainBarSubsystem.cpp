...
void ChainBarSubsystem::postLoopTask() {
    ...
    if (abs(m_requestedChainBarSpeed) > 0.01) {
        ...
        m_chainBarMotor.Set(ControlMode::MotionMagic, ((chainBarAngle + m_chainBarOffset.Get()) * 4096.0 / 360));
    }
    ...
    if (abs(m_requestedRotationSpeed) > 0.01) {
        ...
        m_rotationMotor.Set(ControlMode::MotionMagic, (rotationAngle + m_rotationOffset.Get()) * 4096.0 / 360);
    }
